...
// Define types (keep existing types)
type ShopkeeperData = {
  uid: string;
  email: string;
  shopName: string;
...
// ENHANCED Offer Banner Component with modern design
const OfferBanner = () => {
  const [currentOffer, setCurrentOffer] = useState(0);
  const fadeAnim = useRef(new Animated.Value(1)).current;
    ...
    <View style={styles.offerBanner}>
      ...
      >
        <View style={styles.offerContent}>
          <Animated.View style={[styles.offerTextContainer, { opacity: fadeAnim }]}>
    ...
    >
      ...
      <View style={styles.productInfo}>
        ...
        <View style={styles.productMeta}>
          ...
          <View style={[styles.categoryTag, { backgroundColor: colors.lightBackground }]}>
            <Text style={styles.productCategory}>#{item.category}</Text>
          </View>
        </View>
...
// ENHANCED Customer Post Card Component
const CustomerPostCard = ({ 
  item, 
  index,
  onContactPress
...
export default function CustomerHome() {
  ...
  // Fetch functions (keep existing logic)
  const fetchProductsAndShopkeepers = async () => {
    try {
      const q = query(collection(db, "products"), orderBy("createdAt", "desc"));
      const snapshot = await getDocs(q);
